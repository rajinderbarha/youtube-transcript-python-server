...
# Define Proxy
PROXIES = {
    "http": "http://45.145.128.109:8085",
    "https": "http://45.145.128.109:8085",
}

def get_transcript_with_proxy(video_id):
    """Fetch transcript using a proxy."""
    session = requests.Session()
    session.proxies.update(PROXIES)

    # Manually fetch transcript using YouTubeTranscriptApi
    ...
    try:
        ...
        return YouTubeTranscriptApi.get_transcript(video_id)  # Fetch normally
    except Exception as e:
...
def get_transcript():
    ...
    try:
        ...
        if not transcript:
            return jsonify({"error": "Failed to fetch transcript via proxy"}), 500
...
def proxy_test():
    """Check if the proxy works by making a request to YouTube."""
